...
> **Esta plataforma opera únicamente en modalidad VERI\*FACTU** (la modalidad
> *no verificable* se ha descartado). El marco AEAT contempla ambas, pero aquí solo
> se implementa la verificable.
...
> Son obligatorios el **encadenamiento por huella (hash)** y la **inclusión del código QR** en la factura. *Verificar contra fuente oficial el detalle.*
...
> **Firma electrónica:** en modalidad VERI\*FACTU la firma del registro **no es
> exigible** — la integridad se respalda con la remisión + la huella encadenada.
> (La firma XAdES solo aplicaría a la modalidad no verificable, descartada.)

### 3.3 QR y URL de cotejo
...
- La factura debe llevar además la leyenda **"VERI\*FACTU"** o "Factura verificable en la sede electrónica de la AEAT".
...
- **Event-driven** con una cola/bus de mensajes para desacoplar generación, remisión y reintentos.
...
| Cliente SOAP | **`soap`** o **`strong-soap`** (o envelope manual con `axios` + `https.Agent` con cert cliente) | mTLS con certificado de cliente (PKCS#12). |
...
flowchart TB
    ...
    SENDER -->|SOAP + cert mTLS| AEAT[(Servicio Web AEAT)]
...
- **Salidas:** `RegistroEvento` (encadenado por huella).
- **Modalidad:** VERI\*FACTU.
...
flowchart BT
    ...
    C --> I[Integración: cliente SOAP + validación]
...
- Mención de la modalidad soportada (VERI\*FACTU).

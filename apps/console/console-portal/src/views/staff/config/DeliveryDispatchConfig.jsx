...
const PROVIDERS = [
  {
    ...
    plain: [
      { key: 'uber_customer_id', label: 'Customer ID', placeholder: 'a1b2c3d4-…' },
    ],
    secrets: [
      { key: 'uber_client_id',      label: 'Client ID' },
      { key: 'uber_client_secret',  label: 'Client Secret' },
      { key: 'uber_webhook_secret', label: 'Webhook Signing Secret' },
    ],
  },
  {
    id: 'glovo',
    name: 'Glovo Partners',
    docs: 'https://api-docs.glovoapp.com/',
    plain: [],
    secrets: [
      { key: 'glovo_api_key',        label: 'API Key' },
      { key: 'glovo_webhook_secret', label: 'Webhook Secret (HMAC)' },
    ],
  },
  {
    id: 'stuart',
    ...
    plain: [],
    secrets: [
      { key: 'stuart_client_id',      label: 'Client ID' },
      { key: 'stuart_client_secret',  label: 'Client Secret' },
      { key: 'stuart_webhook_secret', label: 'Webhook Secret' },
    ],
  },
]

export default function DeliveryDispatchConfig() {
  const { toast } = useApp()
  ...
  function reload() {
    setLoading(true)
    api.get('/api/delivery-dispatch/admin/config')
      .then((r) => {
        const data = r?.data ?? []
        setConfig(data)
        const next = {}
  ...
  async function save() {
    ...
    try {
      ...
      await api.patch('/api/delivery-dispatch/admin/config', body)
      toast('Delivery-dispatch configurado')
      reload()
    } catch (err) {
  ...
  return (
    <div className="p-8 max-w-4xl fade-up">
      <div className="mb-8">
        <div className="text-[12px] uppercase tracking-[0.18em] text-ink3 mb-2">Configuración / Delivery dispatch</div>
        <h1 className="font-display text-[44px] leading-none tracking-tight">
          <span className="italic font-normal">Carriers de reparto</span>
        </h1>
        <p className="text-ink3 mt-3 max-w-2xl">
          Credenciales de plataforma para los proveedores de reparto externos (Uber Direct, Glovo, Stuart).
          Las claves se guardan cifradas. La configuración por-tenant (zonas, fees, pickup address) vive en
          el panel de cada tenant.
        </p>
      </div>

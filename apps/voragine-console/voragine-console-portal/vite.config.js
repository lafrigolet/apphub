...
export default defineConfig({
  ...
  server: {
    port: 5177,
    host: true,
    allowedHosts: ['voragine-console.hulkstein.local'],
    proxy: {
      ...
      // works the same as via voragine-console.hulkstein.local:8080.
      '/api': {
        target: 'http://nginx:80',
        changeOrigin: true,
        configure: (proxy) => {
          proxy.on('proxyReq', (proxyReq) => {
            proxyReq.setHeader('Host', 'voragine-console.hulkstein.local')
          })
        },
      },
    },

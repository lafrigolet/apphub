...
export default function TenantEmail() {
  const { toast } = useApp()
  const [items, setItems] = useState(null)
  const [domain, setDomain] = useState('')
  const [creating, setCreating] = useState(false)
  const [busyId, setBusyId] = useState(null)
  const [editing, setEditing] = useState(null) // { id, defaultFromLocal, defaultFromName, replyToAddress }

  function reload() {
    setItems(null)
    api.get('/api/notifications/email-domains')
  ...
  async function createDomain(e) {
    ...
    setCreating(true)
    try {
      await api.post('/api/notifications/email-domains', { domain: domain.trim() })
  ...
  async function verify(id) {
    setBusyId(id)
    try {
      const r = await api.post(`/api/notifications/email-domains/${id}/verify`, {})
  ...
  async function remove(id) {
    ...
    setBusyId(id)
    try {
      await api.delete(`/api/notifications/email-domains/${id}`)
  ...
  async function saveDefaults() {
    ...
    try {
      await api.patch(`/api/notifications/email-domains/${editing.id}`, {
    ...
    <div className="p-8 max-w-4xl fade-up">
      <div className="mb-8">
        ...
        <p className="text-ink3 mt-3 max-w-2xl">
          Configura los dominios desde los que tu tenant enviará correos.
          La plataforma firma cada email con DKIM de tu dominio para que llegue
          autenticado al destinatario.
        </p>
      </div>

      <form onSubmit={createDomain} className="bg-white border border-line rounded-xl shadow-card p-6 mb-8">
        <div className="font-display text-[20px] mb-1">Añadir dominio</div>
      ...
      {items === null
        ...
        : items.length === 0
          ? <div className="p-10 dotted text-center text-ink3 text-sm">No has configurado ningún dominio todavía.</div>
          : (
                ...
                return (
                  <div key={d.id} className="bg-white border border-line rounded-xl shadow-card">
                    <div className="px-5 py-4 border-b border-line flex items-center justify-between">
                      <div>
                        <div className="font-display text-[20px] font-mono">{d.domain}</div>

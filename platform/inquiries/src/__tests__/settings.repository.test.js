...
describe('upsert', () => {
  it('INSERT … ON CONFLICT (app_id, tenant_id) DO UPDATE … RETURNING *', async () => {
    ...
    await repo.upsert(c, {
      ...
      userThanksSubject: 'Gracias', userThanksBody: 'Cuerpo',
    })
    const [sql, params] = c.query.mock.calls[0]
    ...
    expect(params).toEqual(['aikikan', 't1', 'box@x.com', 'r@x.com', 'Gracias', 'Cuerpo'])
  })
  ...
  it('campos opcionales ausentes → null en los params', async () => {
    ...
    expect(params).toEqual(['aikikan', 't1', 'box@x.com', null, null, null])
  })

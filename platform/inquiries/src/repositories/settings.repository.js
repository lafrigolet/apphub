...
export async function findByAppTenant(client, appId, tenantId) {
  const { rows } = await client.query(
    `SELECT app_id, tenant_id, contact_inbox_email, reply_to_email,
            user_thanks_subject, user_thanks_body, retention_days,
...
export async function upsert(client, { appId, tenantId, contactInboxEmail, replyToEmail, userThanksSubject, userThanksBody, retentionDays }) {

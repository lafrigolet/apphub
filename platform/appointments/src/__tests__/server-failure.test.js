// platform-appointments — rutas de fallo del boot.
// server.js invoca start() al importarse y termina con start().catch(exit(1)).
// Aquí re-importamos con vi.resetModules() bajo mocks que fuerzan fallo en
// distintos puntos: contrato de módulo (loadModule throw) → start().catch;
// y app.listen rechazando → catch interno → process.exit(1).
...
describe('platform-appointments — boot failure paths', () => {
  it('módulo sin register() válido → loadModule throws → start().catch → console.error + exit(1)', async () => {
    const real = modulesMock.services.register
    modulesMock.services.register = undefined
    vi.resetModules()
    await import('../server.js')
    await vi.waitFor(() => { if (!exitSpy.mock.calls.length) throw new Error('exit not called') }, { timeout: 5000 })
    expect(errorSpy).toHaveBeenCalledWith('Failed to start platform-appointments:', expect.any(Error))
    expect(exitSpy).toHaveBeenCalledWith(1)
    modulesMock.services.register = real
  })

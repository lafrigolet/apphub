// platform-appointments server — boot coherente de los 8 módulos del monolito.
...
const MODULES = ['services', 'resources', 'bookings', 'availability', 'intake-forms', 'telehealth', 'packages', 'practitioner-payouts']
...
const { fastifyApp, fastifyFactory, plugins, zodMocks, appGuardMock, createPoolMock, createRedisMock, modulesMock, envMock, loggerMock } = vi.hoisted(() => {
  ...
  const modulesMock = Object.fromEntries(['services', 'resources', 'bookings', 'availability', 'intake-forms', 'telehealth', 'packages', 'practitioner-payouts'].map((m) => [m, mk()]))
  ...
  const envMock = {
    NODE_ENV: 'test', PLATFORM_APPOINTMENTS_PORT: 3300, REDIS_URL: 'redis://localhost:6379',
    MIGRATION_DATABASE_URL: 'postgresql://x:y@localhost:5432/test',
    ...Object.fromEntries(['SERVICES', 'RESOURCES', 'BOOKINGS', 'AVAILABILITY', 'INTAKE_FORMS', 'TELEHEALTH', 'PACKAGES', 'PRACTITIONER_PAYOUTS'].map((m) => [`DATABASE_URL_${m}`, `postgresql://${m}:s@localhost:5432/test`])),
  }
...
vi.mock('@apphub/platform-intake-forms', () => modulesMock['intake-forms'])
vi.mock('@apphub/platform-telehealth', () => modulesMock.telehealth)
vi.mock('@apphub/platform-packages', () => modulesMock.packages)
vi.mock('@apphub/platform-practitioner-payouts', () => modulesMock['practitioner-payouts'])
...
describe('platform-appointments — boot sequence', () => {
  ...
  it('createPool una vez por módulo (los 8 tienen schema)', () => {
  ...
  it('/health público + listen en :3300 host 0.0.0.0', () => {
    const healthCall = fastifyApp.get.mock.calls.find((c) => c[0] === '/health')
    expect(healthCall?.[1]).toMatchObject({ config: { public: true } })
    expect(fastifyApp.listen).toHaveBeenCalledWith({ port: 3300, host: '0.0.0.0' })
  })
  ...
  it('GET /health handler devuelve status ok + lista de módulos', async () => {
    ...
    expect(r.service).toBe('platform-appointments')
  ...
  it('rate-limit errorResponseBuilder devuelve el shape esperado', () => {
    const rateLimitEntry = fastifyApp.register.mock.calls.find((c) => c[0] === plugins.rateLimit)
...
describe('platform-appointments — notFound + error handler', () => {
...
describe('platform-appointments — shutdown handler', () => {
  let exitSpy
  beforeAll(() => {
  ...
  it('SIGTERM cierra app, vacía pools y llama redis.quit + exit(0)', async () => {
    const sigtermCall = process.on.mock?.calls
    // Capturamos los listeners reales registrados por server.js.
    const handler = sigtermListener('SIGTERM')
  ...
  it('SIGINT registrado y dispara shutdown', async () => {
    const handler = sigtermListener('SIGINT')
    expect(handler).toBeDefined()
    try { await handler() } catch { /* exit mock throws */ }
    expect(fastifyApp.close).toHaveBeenCalled()
  })
...
// Helper: lee los listeners reales que server.js registró en process.
function sigtermListener(signal) {

...
// el UUID del objeto no es adivinable y el RLS sigue aplicando. Registra el
// acceso (userId null) y publica el evento igual que la variante autenticada.
export async function getPublicDownloadUrl({ appId, tenantId }, id, ttlSeconds = 300, access = {}) {
  const obj = await withTenantTransaction(pool, appId, tenantId, null, async (c) => {
    ...
    const kindCfg = getKind(row.kind)
    if (!kindCfg?.public) throw new ForbiddenError('object is not publicly downloadable')
    if (row.status !== 'uploaded') {
      throw new ConflictError(`object is ${row.status}, no download available`)
    }
    await repo.insertAccessLog(c, appId, tenantId, {
      objectId: row.id, kind: row.kind, action: 'download',
      userId: null, ip: access.ip ?? null, userAgent: access.userAgent ?? null,
    })
    return row
  })
  const rawUrl = await presignGetWithDisposition(ensureClient(), {
  ...
  await publish({
    type: 'storage.object.downloaded',
    payload: {
      appId, tenantId, objectId: obj.id, kind: obj.kind,
      userId: null, ip: access.ip ?? null, userAgent: access.userAgent ?? null,
    },
  })
  return {
